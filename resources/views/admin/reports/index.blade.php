    ...
    <x-lms-page-shell class="lms-report-shell space-y-5">
        ...
        <div class="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                @foreach([
                    ...
                    <a href="{{ route($item['route']) }}" class="block group">
                        <x-lms-panel class="h-full transition hover:border-blue-200">
                            <div class="flex items-start gap-3">
                                <span class="lms-section__icon shrink-0" aria-hidden="true">
                                    <x-lms-icon :name="$item['icon']" class="w-5 h-5" />
                                </span>
                                <div>
                                    <h2 class="font-semibold text-slate-800 group-hover:text-brand-600 transition">{{ $item['title'] }}</h2>
