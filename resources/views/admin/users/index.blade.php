<x-app-layout>
    <div class="user-management-wrapper lms-page-shell space-y-5">
        <x-lms-page-header title="User Management" subtitle="Manage administrators, instructors, and participants">
            <x-slot:actions>
                <a href="{{ route('admin.users.create') }}" class="lms-btn-primary btn-3d">+ Create User</a>
            </x-slot:actions>
        </x-lms-page-header>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="stat-card p-4">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-xs text-slate-400 uppercase">Instructors</p>
        ...
        <div class="filter-card p-4">
            <form method="GET" class="flex flex-wrap gap-3 items-end">
                ...
                <div class="w-40">
                    <label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Role</label>
                ...
                <div class="flex gap-2">
                    <button type="submit" class="lms-btn-primary btn-3d">Search</button>
                    <a href="{{ route('admin.users.index') }}" class="lms-btn-secondary btn-3d">Reset</a>
                </div>
            </form>
        </div>
                        ...
                        @forelse($users as $user)
                            <tr class="transition">
                                ...
                                <td class="px-4 py-3 text-center">
                                    @if($user->role == 'admin')
                                        <span class="lms-badge lms-badge--danger">{{ ucfirst($user->role) }}</span>
                                    @elseif($user->role == 'instruktur')
                                        <span class="lms-badge lms-badge--info">{{ ucfirst($user->role) }}</span>
                                    @else
                                        <span class="lms-badge">{{ ucfirst($user->role) }}</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-center text-slate-600 dark:text-slate-300 truncate max-w-[120px]">{{ $user->phone ?: '-' }}</td>
                                <td class="px-4 py-3 text-center">
                                    @if($user->approval_status == 'approved')
                                        <span class="lms-badge lms-badge--success">Approved</span>
                                    @elseif($user->approval_status == 'rejected')
                                        <span class="lms-badge lms-badge--danger">Rejected</span>
                                    @else
                                ...
                                <td class="px-4 py-3 text-center">
                                    ...
                                    @else
                                        <span class="lms-badge">Inactive</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex justify-center gap-1.5 flex-wrap">
                                        <a href="{{ route('admin.users.show', $user) }}" class="action-btn px-2 py-1 bg-sky-500 hover:bg-sky-600 text-white rounded-md text-xs">View</a>
                                        <a href="{{ route('admin.users.edit', $user) }}" class="action-btn px-2 py-1 bg-amber-500 hover:bg-amber-600 text-white rounded-md text-xs">Edit</a>
                                        <form method="POST" action="{{ route('admin.users.destroy', $user) }}" data-lms-confirm="Delete this user?" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="action-btn px-2 py-1 bg-red-500 hover:bg-red-600 text-white rounded-md text-xs">Del</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
