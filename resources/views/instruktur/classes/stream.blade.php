<x-app-layout>
<div class="stream-wrapper lms-module-shell space-y-6">
        <x-lms-page-header
            :title="$class->title"
        ...
        <div class="grid lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-1 space-y-5">
                ...
                <div class="sidebar-card dashboard-card bg-white rounded-xl p-5 shadow-md border border-slate-200">
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex justify-between"><span class="text-slate-500">{{ __('lms.common.capacity') }}</span><span class="font-medium"><span class="text-emerald-600">{{ $class->participants->count() }}</span> / {{ $class->quota }}</span></div>
                ...
                <div class="sidebar-card dashboard-card bg-white rounded-xl p-5 shadow-md border border-slate-200">
                    <h3 class="text-base font-semibold text-slate-800 mb-4 flex items-center gap-2">
                        <span>⚡</span> {{ __('lms.common.quick_actions') }}
                    </h3>
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('instruktur.assignments.create', $class) }}" class="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 transition text-slate-700 group">
                            <span class="text-xl group-hover:scale-110 transition">➕</span><span class="font-medium">{{ __('lms.common.new_assignment') }}</span>
                        </a>
                        <a href="{{ route('instruktur.attendances.index', $class) }}" class="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 transition text-slate-700 group">
                            <span class="text-xl group-hover:scale-110 transition">📅</span><span class="font-medium">{{ __('lms.common.manage_attendance') }}</span>
                        </a>
                        <a href="{{ route('instruktur.classes.add-student', $class) }}" class="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 transition text-slate-700 group">
                            <span class="text-xl group-hover:scale-110 transition">👥</span><span class="font-medium">{{ __('lms.common.manage_students') }}</span>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-3 space-y-5">
                ...
                <div class="form-card dashboard-card bg-white rounded-xl p-5 shadow-md border border-slate-200">
                    <h3 class="text-base font-semibold text-slate-800 mb-4 flex items-center gap-2">
                        <span>📢</span> {{ __('lms.common.post_announcement') }}
                    </h3>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <a href="{{ route('instruktur.materials.index', $class) }}" class="lms-stream-action">
                        <span class="lms-stream-action__icon">📄</span>
                        <p class="lms-stream-action__title">{{ __('lms.common.add_material') }}</p>
                        <p class="lms-stream-action__desc">{{ __('lms.common.upload_resources') }}</p>
                    </a>
                    <a href="{{ route('instruktur.assignments.create', $class) }}" class="lms-stream-action">
                        <span class="lms-stream-action__icon">📝</span>
                        <p class="lms-stream-action__title">{{ __('lms.common.new_assignment') }}</p>
                        <p class="lms-stream-action__desc">{{ __('lms.common.create_tasks') }}</p>
                    </a>
                    <a href="{{ route('instruktur.attendances.index', $class) }}" class="lms-stream-action">
                        <span class="lms-stream-action__icon">📅</span>
                        <p class="lms-stream-action__title">{{ __('lms.common.take_attendance') }}</p>
                ...
                <div class="lms-timeline space-y-4">
                    ...
                    <div class="timeline-item lms-timeline-item--announcement dashboard-card bg-white rounded-xl p-5 shadow-md border border-slate-200" id="announcement-{{ $announcement->id }}">
                    ...
                    @empty <div class="timeline-item dashboard-card bg-white rounded-xl p-6 text-center text-slate-400">{{ __('lms.common.no_announcements') }}</div> @endforelse

                    <!-- Materials -->
                    @forelse($materials as $material)
                    <div class="timeline-item lms-timeline-item--material dashboard-card bg-white rounded-xl p-5 shadow-md border border-slate-200">
                        <div class="flex items-start justify-between">
                            <div class="flex gap-3"><div class="w-10 h-10 rounded-full bg-gradient-to-r from-emerald-400 to-teal-400 flex items-center justify-center text-white">📖</div><div><p class="font-semibold text-slate-800">{{ $material->creator?->name }}</p><p class="text-xs text-slate-400">{{ __('lms.common.meeting') }} {{ $material->meeting_number }} • {{ $material->created_at->diffForHumans() }}</p></div></div>
                        ...
                        <div class="flex gap-2 mt-2">@if($material->file_path)<span class="text-xs bg-slate-100 px-2 py-0.5 rounded">📎 {{ __('lms.common.attachment') }}</span>@endif @if($material->youtube_url)<span class="text-xs bg-red-100 px-2 py-0.5 rounded">🎥 {{ __('lms.material.youtube') }}</span>@endif</div>
                    </div>
                    @empty <div class="timeline-item dashboard-card bg-white rounded-xl p-6 text-center text-slate-400">{{ __('lms.common.no_materials') }}</div> @endforelse

                    <!-- Assignments -->
                    @forelse($assignments as $assignment)
                    @php $submittedCount = $assignment->submissions->count(); $gradedCount = $assignment->submissions->whereNotNull('score')->count(); @endphp
                    <div class="timeline-item lms-timeline-item--assignment dashboard-card bg-white rounded-xl p-5 shadow-md border border-slate-200">
                        <div class="flex items-start justify-between">
                            <div class="flex gap-3"><div class="w-10 h-10 rounded-full bg-gradient-to-r from-purple-400 to-pink-400 flex items-center justify-center text-white">📝</div><div><p class="font-semibold text-slate-800">{{ $assignment->creator?->name }}</p><p class="text-xs text-slate-400">{{ __('lms.common.due_label') }} {{ $assignment->deadline->format('d M Y H:i') }}</p></div></div>
                        ...
                        @if($assignment->attachment)<a href="{{ route('secure.assignments.attachment', [$class, $assignment]) }}" class="text-xs text-blue-600 mt-2 inline-block hover:underline">📎 {{ __('lms.common.download') }}</a>@endif
                        @if($submittedCount > 0)
                    ...
                    @empty <div class="timeline-item dashboard-card bg-white rounded-xl p-6 text-center text-slate-400">{{ __('lms.common.no_assignments') }}</div> @endforelse
                </div>
            </div>
        </div>
    </div>
