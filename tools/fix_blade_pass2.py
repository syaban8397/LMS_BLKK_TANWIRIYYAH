...
def fix_section_attrs(text: str) -> str:
    replacements = [
        (
            ':title=__(\'lms.attendance.sessions\') :description=__(\'lms.attendance.index_table_meta\') icon=\\"clipboard\\" compact',
            ':title="__(\'lms.attendance.sessions\')" :description="__(\'lms.attendance.index_table_meta\')" icon="clipboard" compact',
        ),
        (
            ':title=__(\'lms.attendance.sessions\') :description=__(\'lms.attendance.index_card_meta\') icon=\\"clipboard\\" compact',
            ':title="__(\'lms.attendance.sessions\')" :description="__(\'lms.attendance.index_card_meta\')" icon="clipboard" compact',
        ),
        (
            ':title=__(\'lms.grade.submissions\') :description=__(\'lms.grade.index_hint\') icon=\\"clipboard\\" compact',
            ':title="__(\'lms.grade.submissions\')" :description="__(\'lms.grade.index_hint\')" icon="clipboard" compact',
        ),
    ]
    ...
    text = re.sub(
        r":title=\__\('lms\.common\.class_list'\) :description=\__\('lms\.common\.total'\) \. ': ' \. \$enrolledClasses->total\(\) icon=\\\"clipboard\\\" compact",
...
def write_profile_edit() -> None:
    return  # profile/edit.blade.php already valid
...
def main() -> None:
    for rel in FILES:
        ...
        t = convert_list_card(fix_section_attrs(ensure_page_shell(orig)))
        if t != orig:
